...
ScaleUniformDragger::ScaleUniformDragger(ScaleMode scaleMode) : Dragger(), _minScale(0.001), _scaleMode(scaleMode)
{
    _projector = new LineProjector(osg::Vec3d(-0.5,-0.5,-0.5),osg::Vec3d(0.5,0.5,0.5));
    setColor(osg::Vec4(0.0f, 1.0f, 0.0f, 1.0f));
    setPickColor(osg::Vec4(1.0f, 1.0f, 0.0f, 1.0f));
}

ScaleUniformDragger::~ScaleUniformDragger()
{
}

bool ScaleUniformDragger::handle(const PointerInfo& pointer, const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
{
        ...
        case (osgGA::GUIEventAdapter::PUSH):
            {
                ...
                if (_projector->project(pointer, _startProjectedPoint))
                {
                    if (_scaleMode == SCALE_WITH_OPPOSITE_HANDLE_AS_PIVOT)
                    {
						...
						{
							_scaleCenter[0] = _projector->getLineEnd()[0];
							_scaleCenter[1] = _projector->getLineEnd()[1];
							_scaleCenter[2] = _projector->getLineEnd()[2];
						}
        ...
        case (osgGA::GUIEventAdapter::DRAG):
            {
                ...
                if (_projector->project(pointer, projectedPoint))
                {
                    ...
                    // Compute scale.
                    double scale = computeScale(_startProjectedPoint,projectedPoint,_scaleCenter[0]);
...
void ScaleUniformDragger::setupDefaultGeometry()
{
