...
//	defaultTracker->Initialize(778.195, 779.430, 324.659, 235.685, -0.333103, 0.173760, 0.000653, 0.001114, 50);
	...
	defaultTracker->SetRefinement(true);
	...
	IplImage* panoramaImage = cvCreateImage(cvSize(WIDTH*3, HEIGHT*1.5), IPL_DEPTH_8U, 1);
	IplImage* panoramaColor = cvCreateImage(cvSize(WIDTH*3, HEIGHT*1.5), IPL_DEPTH_8U, 3);
	...
	calibration->Initialize(1029.400, 1028.675, 316.524, 211.395, -0.206360, 0.238378, 0.001089, -0.000769);
	...
	while(processing)
	{
		...
		defaultTracker->SetFeatureExtractThreshold(fastThreshold);
		...
		case 'E':
			...
			defaultTracker->SetFeatureExtractThreshold(30);
