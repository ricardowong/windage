...
class OpenSURFdetectorTest : public windageTest
{
private:
	IplImage* grayImage;

public:
	OpenSURFdetectorTest() : windageTest("OpenSURFdetector Test", "OpenSURFdetector")
	{
		grayImage = NULL;
		this->Do();
	}
	~OpenSURFdetectorTest()
	{
	...
	bool TestMemoryRelease(std::string* message)
	{
		...
		// Feature Point
		windage::Algorithms::OpenSURFdetector* surfDetector1 = new windage::Algorithms::OpenSURFdetector();
		p1 = (void*)surfDetector1;
		surfDetector1->DoExtractKeypointsDescriptor(grayImage);
		delete surfDetector1;

		windage::Algorithms::OpenSURFdetector* surfDetector2 = new windage::Algorithms::OpenSURFdetector();
	...
	bool TestAlgorithm(std::string* message)
	{
		...
		windage::Algorithms::OpenSURFdetector surfDetector;

		cvNamedWindow("OpenSURF detector");
		for(int i=1; i<10; i++)
		{
			double threshold = 0.000004f * (double)(pow(2.0, i));
			...
			sprintf_s(tempMessage, "OpenSURF threshold : %.8lf", threshold);
			windage::Utils::DrawTextToImage(resultImage, cvPoint(10, 20), 0.7, tempMessage);

			cvShowImage("OpenSURF detector", resultImage);
			cvWaitKey(1);
		}
	...
	bool Terminate(std::string* message)
	{
		...
		if(grayImage) cvReleaseImage(&grayImage);
		grayImage = NULL;
		cvDestroyWindow("OpenSURF detector");
