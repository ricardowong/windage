...
bool flip = true;
CvCapture* capture;
windage::Frameworks::MultiplePlanarObjectTracking* tracker = NULL;
...
std::vector<osg::MatrixTransform*> localCoordinators;
...
//	tracker->DrawDebugInfo(input);
	...
	{
		arTool->AttatchCameraParameter(tracker->GetCameraParameter(i));
		osg::Matrixd modelView = windageARTool::ForOSG::ConvertOSGMatrix(arTool->GetModelViewMatrix());
		localCoordinators[i]->setMatrix(modelView);
	}	
