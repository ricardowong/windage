...
#ifndef _QUASI_NEWTON_METHOD_H_
#define _QUASI_NEWTON_METHOD_H_
...
namespace windage
{
	enum QUASI_METHOD{SR1=0, BFGS, DFP};
	class QuasiNewtonMethod : public MultivariateOptimization
	{
	private:
		int dimension;
		QUASI_METHOD method;

		Matrix2 FindHessian(Vector2 xk, Vector2 sk, Matrix2 B);
	...
	public:
		QuasiNewtonMethod();
		inline ~QuasiNewtonMethod(){};

		inline void SetDemesion(int dimension){this->dimension = dimension;};
		inline void SetMethod(QUASI_METHOD method){this->method = method;};
