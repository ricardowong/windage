...
#ifndef _OPENCV_RANSAC_ESTIMATOR_H_
#define _OPENCV_RANSAC_ESTIMATOR_H_
...
namespace windage
{
	namespace Algorithms
	{
		...
		/**
		 * @brief	class for camera pose estimation in 3D using opencv find extrinsic matrix & RANSAC
		 * @author	Woonhyuk Baek
		 */
		class DLLEXPORT OpenCVRANSACestimator : public PoseEstimator
		{
		protected:
			double confidence;
			int maxIteration;
		public:
			virtual char* GetFunctionName(){return "OpenCVRANSACestimator";};
			OpenCVRANSACestimator() : PoseEstimator()
			{
				this->reprojectionError = 2.0;
				this->confidence = 0.995;
				this->maxIteration = 1000;
			}
			virtual ~OpenCVRANSACestimator()
			{
			}
...
#endif // _OPENCV_RANSAC_ESTIMATOR_H_