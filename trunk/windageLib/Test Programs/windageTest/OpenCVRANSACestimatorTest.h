...
class OpenCVRANSACestimatorTest : public windageTest
{
private:
	IplImage* grayImage;
...
public:
	OpenCVRANSACestimatorTest() : windageTest("OpenCVRANSACestimator Test", "OpenCVRANSACestimator")
	{
		grayImage = NULL;
	...
	bool TestMemoryRelease(std::string* message)
	{
		...
		windage::Algorithms::OpenCVRANSACestimator* estimator1 = new windage::Algorithms::OpenCVRANSACestimator();
		p1 = (void*)estimator1;
		estimator1->AttatchCameraParameter(this->calibration);
		estimator1->AttatchReferencePoint(&this->referencePoints);
		estimator1->AttatchScenePoint(&this->scenePoints);
		estimator1->Calculate();
		delete estimator1;

		windage::Algorithms::OpenCVRANSACestimator* estimator2 = new windage::Algorithms::OpenCVRANSACestimator();
		p2 = (void*)estimator2;
		estimator2->AttatchCameraParameter(this->calibration);
	...
	bool TestAlgorithm(std::string* message)
	{
		...
		cvNamedWindow("OpenCV RANSAC estimator");

		for(unsigned int i=0; i<this->referencePoints.size(); i++)
		{
			windage::Vector3 pt = this->referencePoints[i].GetPoint();
			pt.z = 0.0;
			this->referencePoints[i].SetPoint(pt);
		}
		
		windage::Algorithms::OpenCVRANSACestimator estimator;
		estimator.AttatchCameraParameter(this->calibration);
		...
		drawRefPoints[0].x = 0.0;	drawRefPoints[0].y = 0.0;		drawRefPoints[0].z = 0.0;
		drawRefPoints[1].x = width; drawRefPoints[1].y = 0.0;		drawRefPoints[1].z = 0.0;
		drawRefPoints[2].x = width; drawRefPoints[2].y = height;	drawRefPoints[2].z = 0.0;
		drawRefPoints[3].x = 0.0;	drawRefPoints[3].y = height;	drawRefPoints[3].z = 0.0;

		for(int i=0; i<4; i++)
		{
			drawScePoints[i] = estimator.ConvertWorldToImage(drawRefPoints[i]);
		...
		cvShowImage("OpenCV RANSAC estimator", resultImage);
		cvWaitKey(1000);
	...
	bool Terminate(std::string* message)
	{
		...
		cvDestroyWindow("OpenCV RANSAC estimator");
